...
NUTS_AUTH    = os.environ.get("NUTS_AUTH_URL", "https://auth.nuts.services").rstrip("/")
GRUB         = os.environ.get("GRUB_URL", "https://grub.nuts.services").rstrip("/")
RETURN_URL   = os.environ.get("RETURN_URL", "https://force.nuts.services/auth")
...
async def login_url():
    return {"url": f"{NUTS_AUTH}/login?return_url={RETURN_URL}"}
...
async def cfg():
    return {
        ...
        "login_url":     f"{NUTS_AUTH}/login?return_url={RETURN_URL}",
    }
...
async def news(
    ...
    if not claims:
        return JSONResponse(
            {"error": "unauthenticated", "login_url": f"{NUTS_AUTH}/login?return_url={RETURN_URL}"},
            status_code=401,
        )
